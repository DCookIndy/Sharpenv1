    angular.module('sharpenApp.controllers', [])
        ...
        bucketListRef.on('value', function(snapshot) {
            ...
            for (var key in data) {
                if (data.hasOwnProperty(key)) {
                    if (data[key].isCompleted || data[key].isMissed == false ) {
                        data[key].key = key;
        ...
        $scope.markMissed = function(key) {
            ...
            itemRef.update({
                ...
                if (error) {
                    ...
                    $rootScope.hide();
                    $rootScope.notify('Thats a bummer');
                }
            });
        };
    ...
    .controller('missedCtrl', function($rootScope, $scope, $window, $firebase) {
        $rootScope.show("Please wait... Processing");
        $scope.list = [];
    ...
    .controller('TrackCtrl', function($scope){
        ...
        $scope.labels = ["September"];
          $scope.series = ['Completed', 'Missed'];
          $scope.data = [
              [24],
              [12]
          ];
